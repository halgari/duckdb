...
//! A storage extension handle. Must be destroyed with `duckdb_destroy_storage_extension` unless registered with a config.
typedef struct _duckdb_storage_extension {
        void *internal_ptr;
} * duckdb_storage_extension;

//! Attach callback information passed to storage extensions.
typedef struct _duckdb_attach_info {
        void *internal_ptr;
} * duckdb_attach_info;

//! Transaction manager callback information passed to storage extensions.
typedef struct _duckdb_transaction_info {
        void *internal_ptr;
} * duckdb_transaction_info;

//! Callback invoked to create a catalog when attaching a storage extension.
typedef void (*duckdb_storage_attach_t)(duckdb_attach_info info);

//! Callback invoked to create a transaction manager for an attached storage extension.
typedef void (*duckdb_storage_create_transaction_t)(duckdb_transaction_info info);
...
/*!
Creates a new storage extension handle.

* @return The created storage extension handle.
*/
DUCKDB_C_API duckdb_storage_extension duckdb_create_storage_extension();

/*!
Destroys a storage extension handle that was not registered with a config.

* @param extension The storage extension handle to destroy.
*/
DUCKDB_C_API void duckdb_destroy_storage_extension(duckdb_storage_extension *extension);

/*!
Associates custom user data with the storage extension.

* @param extension The storage extension handle.
* @param extra_info User data that will be available in callbacks.
* @param destroy Optional destructor for the user data.
*/
DUCKDB_C_API void duckdb_storage_extension_set_extra_info(duckdb_storage_extension extension, void *extra_info, duckdb_delete_callback_t destroy);

/*!
Retrieves the user data associated with the storage extension.

* @param extension The storage extension handle.
* @return The user data pointer or nullptr.
*/
DUCKDB_C_API void *duckdb_storage_extension_get_extra_info(duckdb_storage_extension extension);

/*!
Sets the attach callback for the storage extension.

* @param extension The storage extension handle.
* @param attach The attach callback.
*/
DUCKDB_C_API void duckdb_storage_extension_set_attach(duckdb_storage_extension extension, duckdb_storage_attach_t attach);

/*!
Sets the transaction manager creation callback for the storage extension.

* @param extension The storage extension handle.
* @param create_transaction The transaction manager callback.
*/
DUCKDB_C_API void duckdb_storage_extension_set_transaction(duckdb_storage_extension extension, duckdb_storage_create_transaction_t create_transaction);

/*!
Registers the storage extension with a configuration object. After registration the extension handle becomes invalid.

* @param config The configuration that will own the storage extension.
* @param type The database type identifier handled by the extension.
* @param extension The storage extension handle.
* @return Returns DuckDBSuccess on success or DuckDBError otherwise.
*/
DUCKDB_C_API duckdb_state duckdb_config_add_storage_extension(duckdb_config config, const char *type, duckdb_storage_extension extension);

/*!
Retrieves the catalog name used for the attachment.

* @param info The attach callback information.
* @return The catalog name.
*/
DUCKDB_C_API const char *duckdb_attach_info_get_name(duckdb_attach_info info);

/*!
Retrieves the path that is being attached.

* @param info The attach callback information.
* @return The database path.
*/
DUCKDB_C_API const char *duckdb_attach_info_get_path(duckdb_attach_info info);

/*!
Updates the path that should be attached.

* @param info The attach callback information.
* @param path The new path to attach.
*/
DUCKDB_C_API void duckdb_attach_info_set_path(duckdb_attach_info info, const char *path);

/*!
Configures the attach callback to use DuckDB's default catalog implementation.

* @param info The attach callback information.
*/
DUCKDB_C_API void duckdb_attach_info_set_duck_catalog(duckdb_attach_info info);

/*!
Reports an error for the attach callback, aborting the attach.

* @param info The attach callback information.
* @param error The error message.
*/
DUCKDB_C_API void duckdb_attach_info_set_error(duckdb_attach_info info, const char *error);

/*!
Retrieves the user data configured on the storage extension.

* @param info The attach callback information.
* @return The user data pointer or nullptr.
*/
DUCKDB_C_API void *duckdb_attach_info_get_extra_info(duckdb_attach_info info);

/*!
Retrieves the client context in which the attach is executed.

* @param info The attach callback information.
* @param out_context Where to store the retrieved client context handle.
*/
DUCKDB_C_API void duckdb_attach_info_get_client_context(duckdb_attach_info info, duckdb_client_context *out_context);

/*!
Retrieves a string attach option if it was provided.

* @param info The attach callback information.
* @param key The option name.
* @param out_value Where to store the option value.
* @return Returns true if the option was found.
*/
DUCKDB_C_API bool duckdb_attach_info_get_option_varchar(duckdb_attach_info info, const char *key, const char **out_value);

/*!
Retrieves a boolean attach option if it was provided.
...
* @return Returns true if the option was found.
*/
DUCKDB_C_API bool duckdb_attach_info_get_option_boolean(duckdb_attach_info info, const char *key, bool *out_value);

/*!
Retrieves an integer attach option if it was provided.
...
* @return Returns true if the option was found.
*/
DUCKDB_C_API bool duckdb_attach_info_get_option_bigint(duckdb_attach_info info, const char *key, int64_t *out_value);

/*!
Retrieves a double attach option if it was provided.
...
* @return Returns true if the option was found.
*/
DUCKDB_C_API bool duckdb_attach_info_get_option_double(duckdb_attach_info info, const char *key, double *out_value);

/*!
Configures the transaction callback to use DuckDB's default transaction manager.

* @param info The transaction manager callback information.
*/
DUCKDB_C_API void duckdb_transaction_info_set_duck_transaction_manager(duckdb_transaction_info info);

/*!
Reports an error for the transaction manager callback.

* @param info The transaction manager callback information.
* @param error The error message.
*/
DUCKDB_C_API void duckdb_transaction_info_set_error(duckdb_transaction_info info, const char *error);

/*!
Retrieves the user data configured on the storage extension for transaction callbacks.

* @param info The transaction manager callback information.
* @return The user data pointer or nullptr.
*/
DUCKDB_C_API void *duckdb_transaction_info_get_extra_info(duckdb_transaction_info info);
